...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["image/*", "audio/*", "application/*"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for processed-media" ON storage.objects FOR SELECT USING (bucket_id = 'processed-media');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for processed-media" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'processed-media');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for processed-media" ON storage.objects FOR UPDATE USING (bucket_id = 'processed-media');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for processed-media" ON storage.objects FOR DELETE USING (bucket_id = 'processed-media');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'processed-media',
        public: true,
        allowed_mime_types: ["image/*", "audio/*", "application/*"],
