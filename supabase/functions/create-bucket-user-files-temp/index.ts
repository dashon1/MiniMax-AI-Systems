...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["image/*", "application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "text/*", "application/json"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for user-files" ON storage.objects FOR SELECT USING (bucket_id = 'user-files');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for user-files" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'user-files');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for user-files" ON storage.objects FOR UPDATE USING (bucket_id = 'user-files');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for user-files" ON storage.objects FOR DELETE USING (bucket_id = 'user-files');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'user-files',
        public: true,
        allowed_mime_types: ["image/*", "application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "text/*", "application/json"],
